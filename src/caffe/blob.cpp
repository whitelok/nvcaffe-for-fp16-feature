...
void Blob::FromProto(const BlobProto& proto, bool reshape) {
  ...
  if (proto.double_data_size() > 0) {
    ...
    data_tensor_->invalidate_others();
      ...
      for (int i = 0; i < count_; ++i) {
          // please refer to caffeParser.cpp:456 in DIT repo for reasons why I do this
          float16 t; t.setx(proto.half_data(i) & 0xFFFF);
          set_value_at(true, i, t);
      }
  ...
  if (proto.double_diff_size() > 0) {
    ...
    diff_tensor_->invalidate_others();
  } else if (proto.half_diff_size()) {
      CHECK_EQ(count_, proto.half_diff_size());
      for (int i = 0; i < count_; ++i) {
          // please refer to caffeParser.cpp:456 in DIT repo for reasons why I do this
          float16 t; t.setx(proto.half_data(i) & 0xFFFF);
          set_value_at(false, i, t);
      }
...
void Blob::ToProto(BlobProto* proto, bool store_in_old_format, bool write_diff) const {
  ...
  if (dt == FLOAT16) {
      const float16 *pdata = cpu_data<float16>();
      for (int i = 0; i < count_; ++i) {
          proto->add_half_data(pdata[i].getx());
      }
      if (write_diff) {
          const float16 *pdiff = cpu_diff<float16>();
          for (int i = 0; i < count_; ++i) {
              proto->add_half_data(pdiff[i].getx());
          }
      }
